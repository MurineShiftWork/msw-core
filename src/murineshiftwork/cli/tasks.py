...
def _list_tasks_filesystem(detailed=False):
    ...
    try:
        ...
        tasks_dirs = (
            [Path(p) for p in _tasks_pkg.__path__]
    ...
    if not tasks_dirs:
        try:
            ...
            tasks_dirs = [Path(p) / "tasks" for p in murineshiftwork.__path__]
        except ImportError:
    ...
    for tasks_dir in tasks_dirs:
        ...
        for item in sorted(tasks_dir.iterdir()):
            if (
                ...
                and (item / "task.py").exists()
                and item.name not in summary
            ):
...
def load_task_module(task_name: str):
    ...
    return importlib.import_module(f"murineshiftwork.tasks.{task_name}.task")
